...
def evaluate(args, policy, tokenizer, dataloader):
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            norm_logprobs = new_logprobs / seq_mask.sum(-1)

            loss.append(-F.logsigmoid(args.rebel.beta * (norm_logprobs[:args.per_device_train_batch_size] - norm_logprobs[args.per_device_train_batch_size:]) - args.rebel.gamma_beta_ratio * args.rebel.beta).mean().reshape(1))
...
if __name__ == '__main__':
    ...
    for update in range(1, args.rebel.num_updates + 1):
        ...
        # save model
        if (update - 1) % args.print_sample_output_freq == 0 and args.output_dir: # !!!!!!! and update > 1
        ...
        for mini_batch_start in range(0, args.local_batch_size, args.per_device_train_batch_size):
            ...
            with accelerator.accumulate(policy):
                ...
                norm_logprobs = new_logprobs / mb_seq_mask.sum(-1)
                loss = -F.logsigmoid(args.rebel.beta * (norm_logprobs[:args.per_device_train_batch_size] - norm_logprobs[args.per_device_train_batch_size:]) - args.rebel.gamma_beta_ratio * args.rebel.beta).mean()
