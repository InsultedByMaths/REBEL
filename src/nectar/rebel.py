...
class TaskHParams:
    ...
    query_dataset: str = "jdchang/nectar_openchat_preprocess"
...
class Args:
    # common args
    exp_name: str = "nectar_rebel"
    """the name of this experiment"""
    seed: int = 555134
    """seed of the experiment"""
    track: bool = True
    """if toggled, this experiment will be tracked with Weights and Biases"""
    wandb_project_name: str = "nectar"
    ...
    # other args
    base_model: str = "openchat/openchat_3.5"
    """the name of the pretrained model to use"""
    offload: bool = True
    """Whether to offload ref policy and reward model to CPU"""
    reward_model: str = "berkeley-nest/Starling-RM-7B-alpha"
    """the name of the trained reward model to use"""
    base_reward_model: str = "meta-llama/Llama-2-7b-chat-hf"
    """base model of the reward model"""
...
class GPTRewardModel(nn.Module):
    def __init__(self, args, model_path):
        ...
        self.PAD_ID = self.tokenizer(self.tokenizer.pad_token)["input_ids"][-1]
    ...
    def forward(
        ...
        for i in range(bs):
            c_inds = (input_ids[i][self.args.task.query_length:] == self.PAD_ID).nonzero()
            c_ind = c_inds[0].item() + self.args.task.query_length if len(c_inds) > 0 else input_ids.shape[1]
...
class MistralBranch(ModelBranch):
...
def get_reward(
    ...
    text = [" ".join([p, r.replace(tokenizer.eos_token, " " + rm_tokenizer.eos_token)]) for p, r in zip(prompt, responses)] # Llama template is <gen> </s> rather than <gen></s>
    ...
    rm_response_tokens = rm_tokenizer(
        ...
        max_length=2048,
        truncation=True,
        add_special_tokens=False, # For LLama2 they add start token. Prompt should already have
        return_tensors='pt'
    )
...
def evaluate(args: Args, reward_model, policy, tokenizer, dataloader, generation_config, sampling=True):
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            query = data["oc_prompt_tokens"].to(device)
            rm_query = data["rm_tokens"]
            ...
            eval_storage.query.extend(data["prompt"])
            # eval_storage.reference_response.extend(data["answers"])
...
if __name__ == '__main__':
    ...
    ref_policy = MistralBranch(policy, num_layers_unfrozen=args.num_layers_unfrozen)
    ...
    # reward model (Starling RM 7B)
    reward_model = GPTRewardModel(args, args.base_reward_model)
    rm_tokenizer = reward_model.tokenizer
    rm_tokenizer.padding_side = 'right'
    rm_tokenizer.truncation_side = 'right'

    directory = snapshot_download(args.reward_model, local_dir="models")
    
    # Randomly doesn't work... so making it go twice
    try:
    ...
    dataset = dataset.with_format("torch", columns=["oc_prompt_tokens", "rm_tokens"])
    ...
    validation_dataset = validation_dataset.with_format("torch", columns=["oc_prompt_tokens", "rm_tokens", "prompt"])
    ...
    for update in range(1, args.rebel.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            queries = data["oc_prompt_tokens"].to(device)
            rm_queries = data["rm_tokens"]
            context_length = queries.shape[1]
